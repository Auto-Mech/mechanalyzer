    ...
    (('H', 'O2'), ('OH', 'O'), (None,)): [
        {'high': (np.array([500, 1000, 1500]),
         ...
         10: (np.array([500, 1000, 1500]),
              np.array([3.57572885e+134, 4.79926202e+143, 2.72670689e+149]))},
        {'high': (np.array([500, 1000, 1500]),
                  [4.6548277231154764e+45, 8.556998184634325e+52, 4.662500917095324e+56]),
         1: (np.array([500, 1000, 1500]),
             [4.6548277231154764e+45, 8.556998184634325e+52, 4.662500917095324e+56]),
         10: (np.array([500, 1000, 1500]),
             [4.6548277231154764e+45, 8.556998184634325e+52, 4.662500917095324e+56])}],
    (('H2', 'O'), ('OH', 'OH'), (None,)): [
        {'high': (np.array([500, 1000, 1500]),
         ...
         1: (np.array([500, 1000, 1500]),
             np.array([3.57572885e+134, 4.79926202e+143, 2.72670689e+149])),
         10: (np.array([500, 1000, 1500]),
              np.array([3.57572885e+134, 4.79926202e+143, 2.72670689e+149]))}, None],
    (('H', 'O'), ('OH',), (None,)): [
        {'high': (np.array([500, 1000, 1500]),
                  np.array([3.57572885e+134, 4.79926202e+143, 2.72670689e+149])),
         1: (np.array([500, 1000, 1500]),
         ...
         10: (np.array([500, 1000, 1500]),
              np.array([3.57572885e+134, 4.79926202e+143, 2.72670689e+149]))},
        {'high': (np.array([500, 1000, 1500]),
                  [1.420849319576619e+96, 2.8405686431169553e+77, 3.4922934313599517e+72]),
         1: (np.array([500, 1000, 1500]),
             [5.8295576381190475e+100, 2.3308958102634265e+82, 4.2985260083885116e+77]),
         10: (np.array([500, 1000, 1500]),
              [2.3917907260059445e+105, 1.912671707993609e+87, 5.2908858341829314e+82])}],
    (('H', 'O'), ('OH',), ('(+M)',)): [
        {'high': (np.array([500, 1000, 1500]),
                  np.array([3.57572885e+134, 4.79926202e+143, 2.72670689e+149])),
         1: (np.array([500, 1000, 1500]),
         ...
         10: (np.array([500, 1000, 1500]),
              np.array([3.57572885e+134, 4.79926202e+143, 2.72670689e+149]))},
        {'high': (np.array([500, 1000, 1500]),
                  [9.813202359645695e+109, 1.569488025355258e+92, 6.512342336821681e+87]),
         1: (np.array([500, 1000, 1500]),
             [4.0262276922599165e+114, 1.2878805345625882e+97, 8.015784887656628e+92]),
         10: (np.array([500, 1000, 1500]),
              [1.6519081983453455e+119, 1.0568008449314422e+102, 9.866312924289953e+97])}],
# (('H', 'O'), ('OH',), ('+O(S)',)): [
...
def test__sort_ktp():
    ...
    # os.path.join(CWD, SPC_NAME), os.path.join(CWD, MECH_NAMES[i]))
    for i, SPC_NAME in enumerate(SPC_NAMES):
        spc_path = os.path.join(CWD, SPC_NAME)
        mech_path = os.path.join(CWD, MECH_NAMES[i])
    ...
    srt_mch = mechanalyzer.parser.mech.sorting(
        mech_info, spc_dct_full, SORT_STR, ISOLATE_SPECIES)
    sortd_idx, cmts_dct, spc_dct = mechanalyzer.parser.mech.sorted_mech(srt_mch)
    al_ktp_dct_sorted = mechanalyzer.parser.mech.reordered_mech(
        al_ktp_dct, sortd_idx)
    rxn_param_dct_sorted = mechanalyzer.parser.mech.reordered_mech(
        rxn_param_dct, sortd_idx)

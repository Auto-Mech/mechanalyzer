...
class SORT_MECH:
    ...
    def __init__(self, mech_info, spc_dct):
        ...
        [formulas_dct, formula_str_lst, rct_names_lst,
        ...
        data = np.array([rct_names_lst, prd_names_lst, rct_names_lst_ordered, prd_names_lst_ordered,
                         R1, R2, molecularity, N_of_prods, formula_str_lst], dtype=object).T
        self.mech_df = pd.DataFrame(data, index=rxn_name_lst, columns=[
                                    'rct_names_lst', 'prd_names_lst', 'rct_names_lst_ord', 'prd_names_lst_ord', 'R1', 'R2', 'molecularity', 'N_of_prods', 'PES'])
    ...
    def sort(self, hierarchy, species_list):
        ...
        if len(species_list) > 0:
            if species_list[-1] == 'SUBMECH':
                ...
                if hierarchy[-1] != 0:
                    print(
                        'error: submech extraction already provides headers. incompatible with other headers. please select 0')
        ...
        try:
            ...
            print(
                'WARNING: The reactions were not sorted according to all desired criteria: missing {}'.format(e))
            return None

        # 2. assign class headers
        # set labels for all the possible criteria
        criteria_all = ['molecularity', 'N_of_prods', 'SPECIES', 'PES',
                        'SUBPES', 'R1', 'MULT', 'RXN_CLASS_BROAD', 'RXN_CLASS_GRAPH']
        labels_all = ['molecularity', 'N_of_prods', 'SPECIES', 'PES', 'SUBPES',
                      'Heavier rct', 'Total multiplicity', 'rxn type broad', 'rxn type']
    ...
    def class_headers(self, hierarchy, labels):
        ...
        try:
            ...
            print(
                'Last line of sorting options must be the N of criteria to be used for class headers')
            exit()
        ...
        if N > 0:
            for name, rxndf in self.mech_df.groupby(hierarchy[:N]):
                ...
                rxnclass = cmts_string(
                    name, labels[hierarchy[:N]], 'class_head')
                idx0 = rxndf.index[0]
                ...
                if N < len(hierarchy)-1:
                    for name2, rxndf2 in rxndf.groupby(hierarchy[N:-1]):
                        rxnclass = cmts_string(
                            name2, labels[hierarchy[N:-1]], 'subclass')
        ...
        for subpes_name, subpes_df in self.mech_df.groupby('SUBPES'):
            ...
            for rxn in subpes_df.index:
                ...
                if len(rct_names) < 3 and len(prd_names) < 3:
                    ...
                    rct_ichs = list(self.spc_dct[rct]['inchi']
                    ...
                    prd_ichs = list(self.spc_dct[prd]['inchi']
            ...
            for rxn in subpes_df.index:
                if rxn_clG_df['RXN_CLASS_GRAPH'][rxn] == 'unclassified':
                    ...
                    try:
                        ...
                        rxn_clG_df['RXN_CLASS_GRAPH'][rxn] = rxn_type_1 + \
                            '-' + rxn_type_2 + ' (WS)'
                    except IndexError:
    ...
    def return_mech_df(self):
        ...
        cmts_df = pd.DataFrame(self.mech_df[['cmts_top', 'cmts_inline']].values, index=new_idx, columns=[
                               'cmts_top', 'cmts_inline'])
...
def find_conn_chnls(pes_rct_names_lst, pes_prd_names_lst, pes_rxn_name_lst):
    ...
    len_rct_prd = np.array(list(map(len, pes_rct_names_lst))) + \
        np.array(list(map(len, pes_prd_names_lst)))
    pes_df = pd.DataFrame(np.array([pes_rct_names_lst, pes_prd_names_lst, len_rct_prd], dtype=object).T, index=np.arange(
        0, len(pes_rxn_name_lst)), columns=['rcts', 'prds', 'N_rcts_prds'])

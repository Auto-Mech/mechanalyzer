...
def read_params(output_str, fit_type, a_conv_factor=1.000, ea_conv_factor=RC):
    ...
    if fit_type == 'single':
        for line in lines:
            if 'Converged Params for single modified Arrhenius' in line:
                params_str = lines[lines.index(line)-4]
                break
    elif fit_type in ('double', 'single-double'):
        for line in lines:
            if 'Converged Params for double modified Arrhenius' in line:
                params_str = lines[lines.index(line)-4]
                break
    ...
    if fit_type in ('double', 'single-double') and params_str:
    ...
    elif fit_type in ('double', 'single-double') and double_fit_success:
        fit_params = [float(param) for param in params_str.split()]
        fit_params[0] *= a_conv_factor
        fit_params[2] *= ea_conv_factor
        fit_params[3] *= a_conv_factor
        fit_params[5] *= ea_conv_factor
    elif fit_type in ('double', 'single-double') and not double_fit_success:
        fit_params = []
